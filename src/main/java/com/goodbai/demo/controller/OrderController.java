...
public class OrderController {
    ...
    public String orderManage(Order order, @PathVariable Integer pageCurrent,
        ...
        try {
            String str = order.getMinOrderTimeStr();
            if (str != null) {
                order.setMinOrderTime(DateUtil.strToDate(str));
            } else {
                order.setMinOrderTime(DateUtil.strToDate(str));
            }
            String str2 = order.getMaxOrderTimeStr();
            if (str2 != null) {
                order.setMaxOrderTime(DateUtil.strToDate(str));
            } else {
                order.setMaxOrderTime(new Date());
            }
        }catch (Exception e){
            e.printStackTrace();
        }

        int rows = orderService.list(order).size();
    ...
    public String RefundManage(Order order, @PathVariable Integer pageCurrent,
        ...
        order.setStart((pageCurrent - 1) * pageSize);
        order.setEnd(pageSize);
        List<Order> orderList = orderService.listRefund(order);
        for (Order order1 : orderList) {
            String orderId = order1.getOrderId();
            if (orderItemService.selectByPrimaryOrderKey(orderId) != null) {
                OrderItem orderItem = orderItemService.selectByPrimaryOrderKey(orderId);
